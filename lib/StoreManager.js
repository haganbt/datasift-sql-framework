	...
	db.drop(function () {
	  ...
	  db.models.twitter.sync(function () {
	    if(err) console.log(err);
	 	...
	 	db.models.twitter_user.sync(function () {
	    if(err) console.log(err);
	  }); 
	});
});
    	...
    	if (body.hasOwnProperty(i)) {
    		var item = JSON.parse(body[i]);
				...
				this.Interaction.create([item.interaction], function (err, items) {
					 	if (err) console.log("Insert failed for INTERACTION: ", err);
				...
				if(item.twitter && !item.twitter.retweet && item.twitter.user){
  				...
  				this.Twitter.create([item.twitter.user], function (err, items) {
  					 	if (err) console.log("Insert failed for TWITTER_USER: ", err);
  					 	console.log(JSON.stringify(items));	
  				});
  			} 
				...
				if(item.twitter && !item.twitter.retweet){
  				...
  				this.Twitter.create([item.twitter], function (err, items) {
  					 	if (err) console.log("Insert failed for TWITTER: ", err);	
  				});
  			}
