...
StoreManager = function StoreManager() {
	...
	this.Twitter_mention	= db.models.twitter_mention;
	this.Twitter_place		= db.models.twitter_place;
	this.Twitter_retweet	= db.models.twitter_retweet;	
	...
	try {
    	...
    	if (body.hasOwnProperty(i)) {
    		var item = JSON.parse(body[i]);
				...
				var id = item.interaction.id; // UUID 
				var ts = utils.dateFormat(item.interaction.created_at); // TS
				...
				if(item.twitter && item.twitter.mentions && item.twitter.mention_ids){
					for (var y in item.twitter.mentions) {
						if (item.twitter.mentions.hasOwnProperty(y) && item.twitter.mention_ids.hasOwnProperty(y)) {
							...
							this.Twitter_mention.create(eachMention, function (err, items) {
				...
				if(item.twitter && item.twitter.retweet){
  				...
  				this.Twitter_retweet.create([item.twitter.retweet], function (err, items) {
	...
	db.load("./models/models", function (err) {  
	  if (err) {
		...
		db.drop(function () {
		  ...
		  db.models.twitter_mention.sync(function (err) {
		    if(err) return err;
		  }); 		  
		  db.models.twitter_place.sync(function (err) {
		    if(err) return err;
		  }); 
		  db.models.twitter_retweet.sync(function (err) {
		    if(err) return err;
		  }); 
