    	...
    	if (body.hasOwnProperty(i)) {
    		var item = JSON.parse(body[i]);
				...
				if(item.twitter){
				  ...
				  var twitterObj = item.twitter;
          
				  // tweet or retweet?
				  if(item.twitter.retweet){
				    twitterObj = item.twitter.retweet;
				    twitterObj.twitter_retweeted_id = item.twitter.retweeted.id;
				  }  

          // set the twitter id to the twitter_id field  
          twitterObj.twitter_id = twitterObj.id;

          // interaction id
          twitterObj.id = id;
          
  				this.Twitter.create([twitterObj], function (err, items) {
  					 	if (err) console.log("Insert failed: ", err);
  					 	//console.log(JSON.stringify(items));	
  				});
  			}	
