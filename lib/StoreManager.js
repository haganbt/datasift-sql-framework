...
StoreManager.fn.process = function (inbound) {
	...
	try {
         ...
         *	]
         * 
         * In this implementation we only keep the first link
         * otherwise the table would include duplicate 
         * interaction id's and may become confusing to
         * interact with.
         * 
				 */
				if(item.links && item.links.url && typeof(item.links.url) === 'object'){
						...
						var eachLink 		= {};
						eachLink.id 		= id;
						eachLink.title	= item.links.url[0].title;
						eachLink.url 		= item.links.url[0].url;
